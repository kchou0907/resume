...
const Parthean: React.FC<SweExperienceBias> = ({ bias = ResumeTypes.FULL }) => {
  ...
  const NOTIFICATIONS_INFRASTRUCTURE_HIGHLIGHT: string = `Designed and 
    implemented a scalable, fault-tolerant notification system leveraging Go, 
    GCP, and Apple's Push Notification Service, handling thousands 
    of concurrent notification requests with high reliability`;
  ...
  switch (bias) {
    case ResumeTypes.FULL:
      highlightedExperiences = [...fullstackBias, ...highlightedExperiences];
      break;
